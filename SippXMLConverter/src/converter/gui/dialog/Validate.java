...
public class Validate extends JDialog implements IConstants {
    ...
    public void updatePane(String[] type, String[] message) {
        colorUAC="#33CC00";
        colorUAS="#33CC00";
        if(type[0].startsWith("Warning"))
            colorUAC = "#FFFF00";
        else if(type[0].startsWith("Error"))
            colorUAC = "#FF0000";
        else if(type[0].startsWith("Fatal Error"))
            colorUAC = "#660000";
        if(type[1].startsWith("Warning"))
            colorUAS = "#FFFF00";
        else if(type[1].startsWith("Error"))
            colorUAS = "#FF0000";
        else if(type[1].startsWith("Fatal Error"))
            colorUAS = "#660000";
        if(type[0].isEmpty()) errorsUAC.setText("<center><h3><u>User Agent Client</u></h3></center><font color="+colorUAC+">XML file OK!</font>");
        else errorsUAC.setText("<center><h3><u>User Agent Client</u></h3></center><a href='"+type[0].replaceFirst("(^.+@)(\\d+:\\d+$)", "$2")+"'>"+type[0]+"</a><br/><font color="+colorUAC+">"+message[0]+"</font>");
        if(type[1].isEmpty()) errorsUAS.setText("<center><h3><u>User Agent Server</u></h3></center><font color="+colorUAC+">XML file OK!</font>");
        else errorsUAS.setText("<center><h3><u>User Agent Server</u></h3></center><a href='"+type[1].replaceFirst("(^.+@)(\\d+:\\d+$)", "$2")+"'>"+type[1]+"</a><br/><font color="+colorUAS+">"+message[1]+"</font>");
    }
