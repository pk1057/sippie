...
public class Operation implements ICheckbox {
    ...
    private static ArrayList<SequentialMap<String,String>> readIn(String pcap, String callID) {
        ...
        try {
            captor = JpcapCaptor.openFile(pcap);
            Packet p;
            do {
                p = captor.getPacket();
            }while(!new String(p.data).isEmpty()&&!getCallID(new String(p.data)).equals(callID));
            
            while(p!=null&&p!=Packet.EOF) {
                ...
                OUTER: for(int i=0;enter.hasMoreTokens();i++) {
                ...
                do {
                    p = captor.getPacket();
                }while(p.data!=null&&!new String(p.data).isEmpty()&&!getCallID(new String(p.data)).equals(callID));
            }
        }
        catch(IOException e) {
    ...
    private static String replaceText(SequentialMap<String,String> sm, int k) {
        ...
        for(int i=1;i<sm.size();i++) {
            ...
            if(key.equalsIgnoreCase("Via")) {
                if(cbViaL[k].isSelected()) erg.append(formatVia("[last_Via:]",k));//+getLastString(sm.get(key)),firstVia,k));
                else erg.append(key+formatVia(sm.get(key),k));
            }
            else if(key.equalsIgnoreCase("From")) {
                if(cbFromL[k].isSelected()) erg.append(formatFrom("[last_From:]"+getLastString(sm.get(key)),k));
                else erg.append(key+formatFrom(sm.get(key),k));
            }
            else if(key.equalsIgnoreCase("To")) {
                if(cbToL[k].isSelected()) erg.append(formatTo("[last_To:]"+getLastString(sm.get(key)),k));
                else erg.append(key+formatTo(sm.get(key),k));
            }
            else if(key.equalsIgnoreCase("Call-ID")) {
                if(cbCallIDL[k].isSelected()) erg.append("[last_Call-ID:]"+getLastString(sm.get(key))+"\n");
                else erg.append("Call-ID: [call_id]\n");
            }
            else if(key.equalsIgnoreCase("CSeq")) {
                if(cbCseqL[k].isSelected()) erg.append("[last_CSeq:]"+getLastString(sm.get(key))+"\n");
                else erg.append(key+replaceCseq(sm.get(key)));
            }
